...
import base64
from ImageProcess.AiModels.AiProc import styleTransform
import numpy as np
import cv2
...
class styleTransform_view(View):
    def post(self, request, *args, **kwargs):
        ...
        # 获取风格编号
        style_num = (int)(request.POST.get('style_num'))
        # 获取图片格式
        ...
        try:
            # OCR识别结果
            result = 'data:image/{0};base64,'.format(img_format) + styleTransform.styleTransform(img_base64, style_num)
            msg['img_proc_result'] = result
            msg['error_code'] = 0
