# -*- coding: utf-8 -*-
"""
Created on Fri Jul 24 23:33:40 2020

@author: 涂晴昊
@information: 美颜，包含磨皮、美白、亮眼、口红。
"""
...
#            raise ValueError('Opencv error reading image "{}" , got None'.format(fname))
    ...
    # 读cv2图片
    def read_and_mark_from_backend(self,img):
        im_bgr=img.copy()
        im_hsv=cv2.cvtColor(im_bgr, cv2.COLOR_BGR2HSV)
        temp_bgr,temp_hsv=im_bgr.copy(),im_hsv.copy()
        fname = "tmp"
...
def face_identify(img):
    mu=Makeup(predictor_path = settings.BASE_DIR + "/ImageProcess/AiModels/resource/shape_predictor_68_face_landmarks.dat")
    im,temp_bgr,faces=mu.read_and_mark_from_backend(img)
    imc=im.copy()
    # cv2.imshow('ori',imc)
    name = "tmp"
...
if __name__=='__main__':
    path='6.jpg'
#    mu=Makeup(predictor_path="C:/Users/76785/Desktop/shape_predictor_68_face_landmarks.dat")
    mu=Makeup(predictor_path="../../backend/ImageProcess/AiModels/resource/shape_predictor_68_face_landmarks.dat")
