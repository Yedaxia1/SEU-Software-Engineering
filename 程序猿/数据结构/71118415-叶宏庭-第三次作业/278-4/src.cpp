...
public:
	...
	void nonrecursivePreorder()
	{
		...
		while (1)
		{
			while (currentNode)
			{
				if (currentNode->rightChlid!=root&&currentNode->rightThread==false)//若右孩子不为root且存在右孩子，右孩子压栈
					s.Push(currentNode->rightChlid);
				Visit(currentNode);//访问当前结点
				if (currentNode->leftChlid != root&&currentNode->leftThread==false)//若左孩子不为root且存在左孩子，currentNode指向左孩子
				{
					currentNode = currentNode->leftChlid;
				}
				else { break; }//到达root，break
			}
			if (s.IsEmpty()) return;
			currentNode = s.Top();
			s.Pop();
